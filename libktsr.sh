...
core_ctl_set() {
	...
	boost_tune() {
		if [[ "$ktsr_prof_en" == "latency" ]] || [[ "$auto_prof" == "latency" ]]; then
		...
		elif [[ "$ktsr_prof_en" == "balanced" ]] && [[ "$auto_prof" == "balanced" ]]; then
			[[ -e "/sys/module/cpu_boost/parameters/dynamic_stune_boost" ]] && {
		...
		elif [[ "$ktsr_prof_en" == "extreme" ]] || [[ "$auto_prof" == "extreme" ]]; then
			[[ -e "/sys/module/cpu_boost/parameters/dynamic_stune_boost" ]] && {
		...
		elif [[ "$ktsr_prof_en" == "pwrsave" ]] || [[ "$auto_prof" == "pwrsave" ]]; then
			[[ -e "/sys/module/cpu_boost/parameters/dynamic_stune_boost" ]] && {
		...
		elif [[ "$ktsr_prof_en" == "game" ]] || [[ "$auto_prof" == "game" ]]; then
			[[ -e "/sys/module/cpu_boost/parameters/dynamic_stune_boost" ]] && {
				write "/sys/module/cpu_boost/parameters/dynamic_stune_boost" "50"
				write "/sys/module/cpu_boost/parameters/dynamic_stune_boost_ms" "700"
				log_i "Tweaked dynamic stune boost"
...
uclamp_tune() {
	...
	} || [[ -e "${cpuctl}top-app/cpu.uclamp.max" ]] && [[ "$ktsr_prof_en" == "balanced" ]] || [[ "$auto_prof" == "balanced" ]] && {
		write "${kernel}sched_util_clamp_min" "1024"
		write "${kernel}sched_util_clamp_max" "1024"
		write "${cpuctl}top-app/cpu.uclamp.max" "max"
...
sched_tune() {
	if [[ "$ktsr_prof_en" == "latency" ]] || [[ "$auto_prof" == "latency" ]]; then
		...
		log_i "Tweaked various kernel parameters to a better overall performance"
	elif [[ "$king_prof_en" == "extreme" ]] || [[ "$auto_prof" == "extreme" ]]; then
		[[ -e "${kernel}sched_child_runs_first" ]] && write "${kernel}sched_child_runs_first" "0"
...
cpu_clk_set() {
	[[ "$1" == "1" ]] && {
		for pl in /sys/devices/system/cpu/cpufreq/policy*/; do
			...
			for i in 576000 633600 652800 691200 710400 748800 768000 787200 806400 825600 844800 852600 864000 902400 940800 960000 979200 998400 1036800 1075200 1113600 1152000 1209600 1459200 1478400 1516800 1651200 1689600 1708800 1766400; do
